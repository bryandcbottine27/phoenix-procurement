...
- F2 adds disabled-by-default Graph notification scaffolding for responsible-officer requested-receipt and stale-sync alerts, including a function-key dry-run preview endpoint, environment-only Graph settings, recipient-map routing, SQL `notification_state` repeat suppression, and SQL integration-ready tests.
...
- Internal API data mode adds `src/apiClient.js`, lazy Firebase loading, production package staging for `dataMode: 'api'`, `backend/db/004_operational_records.sql` plus `/api/records` routes for closed-environment browser testing without Firebase, `backend/db/005_app_counters.sql` for API-mode generated references, and `backend/db/006_order_merge_indexes.sql` for the merged order read/head path.
- The merged order API path is wired: `/api/records` and `/api/records/orders` combine SQL ERP rows with active Phoenix order overlays, strip ERP-owned fields on order overlay writes, support synthetic ERP-only ids for the first operational edit, and are covered by SQL sync-to-records E2E integration.

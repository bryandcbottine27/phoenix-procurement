...
- F5 adds read-only `GET /api/worklists/unclassified` for unclassified/unmapped/currency-ambiguous sync exception worklists, with suggested actions for the future admin UI. The browser admin screen and one-click write actions remain deferred because the handoff also says browser untouched unless explicitly approved.

...
- Internal API data mode in `src/apiClient.js`; when `APP_CONFIG.dataMode === 'api'`, `PXStore` routes normal business writes to the backend operational record API instead of Firestore. The browser loads one initial `/api/records` snapshot, then uses `/api/records/heads` to refetch only collections whose SQL head changed.
...
- Local backend runtime proof is environment-dependent, but Backend v1 has been proven locally against Docker SQL Server in this Codex desktop environment. Bundled Node/pnpm and package-managed Azure Functions Core Tools are available; SQL Server LocalDB, `sqlcmd`, and global Azure Functions Core Tools are not installed. Validation used Docker SQL Server on `localhost:14333`, applied `backend/db/001_init.sql` through `backend/db/005_app_counters.sql`, returned a healthy `/api/health` SQL response, executed `/api/records` and `/api/counters/{counterKey}/next` through the local Functions host, and executed backend SQL integration tests with no skips. A full local Functions host still needs reachable `AzureWebJobsStorage` or Azurite to avoid timer/storage health warnings.
- Browser/client-side permission checks are not sufficient for production security. API mode now has a backend operational-record write gate that resolves `x-phoenix-user` to the stored officer role and re-enforces the permission matrix plus record validation server-side. Final pilot/go-live still needs IT-owned identity and gateway/API policy so the asserted user header is trusted and cannot be spoofed.
...
- API mode uses a generic SQL JSON operational record table as an internal-server bridge. It is suitable for closed-environment testing, and now avoids routine full-store reloads by capping `status_log`, exposing collection heads, and refetching changed collections only. A future normalized SQL operational schema may still be required for long-term reporting/governance.

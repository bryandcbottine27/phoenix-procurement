...
- The root browser app still has no package-managed frontend build, lint, or TypeScript configuration. Backend v1 now has its own `backend/package.json`, TypeScript build, package-managed Azure Functions Core Tools dependency, `mssql`, SQL DDL migration scaffold, fixture source adapter, and `node:test` unit tests. Current browser validation remains Python build, generated data dictionary check, Node syntax checks, invariant checks for structure/status maps/pre-test data-integrity/package zip hygiene, and local Node regression checks for permission/security/import/Data Quality/My Work contracts.

...
**All creates/updates/archives go through `window.PXStore`** (now in its own `firestoreStore.js`). No module calls `addDoc`/`updateDoc`/`setDoc`/`deleteDoc` directly. **There are no hard deletes of business records** — removal is soft `archiveRecord` (reversible via `restoreRecord`). The only generated-reference exception is the per-entity RFP counter: demo/Firebase mode uses an atomic Firestore `runTransaction`, while API mode uses the backend SQL counter endpoint. **PXStore is validation-aware**: it runs `PXValidators` before create/update (errors block, warnings logged), so validation is inherited even if a form forgets to call it.
...
payment approval routing, validator guardrails, ERP import classification, Data Quality timing, and
My Work computed actions. Keep broad structural string checks in `check_invariants.py`; keep
behaviour-style source checks in `regression_checks.js`.

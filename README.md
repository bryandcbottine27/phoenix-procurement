...
later) — it is **not** a replacement ERP. The demo package uses Firebase/Firestore
for testing. The production package is now staged for the internal SQL/API backend
path, with no Firebase dependency in API data mode.
...
```
core.js              foundation: REF config (incl. demoResetEnabled), Firebase/API mode,
                     state, PXUtils helpers, navigation, modal, live subscriptions
...
apiClient.js         PXApiClient — internal SQL/API adapter used only when
                     APP_CONFIG.dataMode === 'api'
firestoreStore.js    PXStore — the ONE write path (centralised, validated, audited);
                     delegates to Firestore or PXApiClient depending on dataMode
...
An isolated Backend v1 scaffold now lives under `backend/` for the Azure Functions
plus SQL Server warehouse connector and internal browser API mode. Demo builds do not
use it. Production builds generated by `tools/package.py` set
`APP_CONFIG.dataMode = 'api'` and call the backend `/api/records` operational-store
routes through `PXApiClient`. Use `backend/README.md` for backend setup, migrations,
fixture-backed source mapping, contract drift tests, and the local `/api/health`
trigger.

### Internal SQL/API production mode

`tools/package.py` stages the production package with:

- `demoMode: false`
- `authMode: 'internal'`
- `dataMode: 'api'`

In that mode Firebase modules are not loaded. The browser loads an initial snapshot
from `GET /api/records`, polls on `APP_CONFIG.apiPollMs`, and sends normal business
writes through `PXStore` to the backend operational record routes. `apiBaseUrl` should
stay `/api` for same-origin IIS/Azure Functions hosting, or be changed to the URL IT
provides. `apiFunctionKey` can carry the Function key during local/private testing;
do not commit real keys.

The current internal auth is a temporary local operator setup stored on the
workstation. It is sufficient for closed-environment testing, but final pilot/go-live
still needs IT to confirm the identity model, such as Entra ID/SSO, Windows-integrated
access, APIM policy, or another internal gateway.

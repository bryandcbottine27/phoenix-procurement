...
(function () {
  ...
  function targetStatus(current, target, direction) {
    if (current === null || current === undefined || current === '') {
      return { status: 'No sample', cls: 'neutral', variance: null };
    }
    const value = Number(current);
    if (!Number.isFinite(value)) return { status: 'No sample', cls: 'neutral', variance: null };
    const variance = direction === 'min' ? value - target : target - value;
    const ok = direction === 'min' ? value >= target : value <= target;
    if (ok) return { status: 'On target', cls: 'success', variance };
    return { status: 'Off target', cls: target === 0 ? 'danger' : 'warn', variance };
  }
  function targetRow(area, metric, current, target, direction, unit, owner, view, action) {
    const result = targetStatus(current, target, direction);
    return {
      area,
      metric,
      current,
      ...
      direction,
      unit: unit || '',
      currentLabel: formatMetricValue(current, unit),
      targetLabel: (direction === 'min' ? '>=' : '<=') + ' ' + formatMetricValue(target, unit),
      varianceLabel: result.variance === null ? '-' : formatMetricValue(Math.abs(Number(result.variance.toFixed ? result.variance.toFixed(1) : result.variance)), unit),
  ...
  function buildKpiTargetVariance(ent) {
    const data = buildManagementData(ent);
    const kpi = window.PXKpi ? window.PXKpi.compute(ent) : {};
    const paymentDays = row => row.daysUntil ?? daysUntil(row.forecastDate || row.dueDate);
    const criticalExceptions = data.exceptions.filter(item => item.check.level === 'danger').length;
    const highOtif = data.otifRisk.filter(row => row.score >= 75).length;
    const clearanceRisk = data.clearance.filter(row => row.score >= 45).length;
    const paymentOverdue = data.payments.rows.filter(row => paymentDays(row) < 0).length;
    const weakSuppliers = data.scorecards.filter(score => score.rating < 60).length;
    const officerPressure = data.workload.filter(row => row.critical || row.dueSoon || row.total >= 8).length;

    return [
      targetRow('Daily control', 'Critical exceptions', criticalExceptions, 0, 'max', '', 'Management', 'exceptions', 'Clear critical exceptions'),
      targetRow('OTIF risk', 'High-risk active orders', highOtif, 0, 'max', '', 'Procurement', 'otifrisk', 'Expedite high-risk orders'),
      targetRow('Logistics', 'Clearance-risk shipments', clearanceRisk, 0, 'max', '', 'Logistics', 'clearance', 'Resolve clearance blockers'),
      targetRow('Finance', 'Overdue payment items', paymentOverdue, 0, 'max', '', 'Finance', 'paymentexposure', 'Review overdue payments'),
      targetRow('Supplier', 'Weak supplier scorecards', weakSuppliers, 0, 'max', '', 'Procurement', 'scorecards', 'Review supplier recovery'),
      targetRow('Workload', 'Officer pressure points', officerPressure, 0, 'max', '', 'Management', 'workload', 'Rebalance open workload'),
      targetRow('KPI', 'OTIF actual', kpi.otifPct, 95, 'min', '%', 'Management', 'kpitrends', 'Review KPI trend'),
      targetRow('KPI', 'MTTO average', kpi.mtto, 7, 'max', 'days', 'Procurement', 'kpitrends', 'Review procurement lead time'),
      targetRow('KPI', 'Average clearance time', kpi.avgClearance, 3, 'max', 'days', 'Logistics', 'kpitrends', 'Review clearance process'),
      targetRow('KPI', 'Shipment lateness average', kpi.avgLateness, 0, 'max', 'days', 'Logistics', 'kpitrends', 'Review late shipment drivers')
    ];
  }

  function checklistRow(cadence, owner, control, count, target, view, action) {
    const result = targetStatus(count, target, 'max');
    return {
      cadence,
      owner,
      control,
      signal: count + (count === 1 ? ' item' : ' items'),
      target: '<= ' + target,
      status: result.status,
      statusClass: result.cls,
      view,
      action
    };
  }

  function buildOperatingChecklist(ent) {
    const data = buildManagementData(ent);
    const paymentDays = row => row.daysUntil ?? daysUntil(row.forecastDate || row.dueDate);
    const criticalExceptions = data.exceptions.filter(item => item.check.level === 'danger').length;
    const supplierChaseDue = data.exceptions.filter(item => /supplier|promise|follow.?up|acknowledg/i.test(item.check.msg || '')).length;
    const clearanceRisk = data.clearance.filter(row => row.score >= 45).length;
    const paymentOverdue = data.payments.rows.filter(row => paymentDays(row) < 0).length;
    const paymentDue = data.payments.rows.filter(row => paymentDays(row) >= 0 && paymentDays(row) <= 7).length;
    const officerPressure = data.workload.filter(row => row.critical || row.dueSoon || row.total >= 8).length;
    const weakSuppliers = data.scorecards.filter(score => score.rating < 60).length;
    const partialControls = data.partials.length;
    const highOtif = data.otifRisk.filter(row => row.score >= 75).length;
    const packRows = managementPackRows(ent).length;

    return [
      checklistRow('Daily', 'Management', 'Clear critical exceptions', criticalExceptions, 0, 'exceptions', 'Review exceptions'),
      checklistRow('Daily', 'Logistics', 'Resolve clearance blockers', clearanceRisk, 0, 'clearance', 'Review clearance'),
      checklistRow('Daily', 'Finance', 'Clear overdue payments', paymentOverdue, 0, 'paymentexposure', 'Review overdue payments'),
      checklistRow('Daily', 'Procurement', 'Act on high OTIF risk', highOtif, 0, 'otifrisk', 'Review OTIF risk'),
      checklistRow('Weekly', 'Procurement', 'Run supplier chase plan', supplierChaseDue, 0, 'supplierchase', 'Open chase plan'),
      checklistRow('Weekly', 'Management', 'Rebalance officer pressure', officerPressure, 0, 'workload', 'Review workload'),
      checklistRow('Weekly', 'Finance', 'Review payments due this week', paymentDue, 0, 'paymentexposure', 'Review due payments'),
      checklistRow('Weekly', 'Logistics', 'Resolve partial shipment controls', partialControls, 0, 'partials', 'Review partial shipments'),
      checklistRow('Monthly', 'Management', 'Export management pack', packRows, 0, 'managementpack', 'Export management pack'),
      checklistRow('Monthly', 'Procurement', 'Review weak supplier scorecards', weakSuppliers, 0, 'scorecards', 'Open scorecards'),
      checklistRow('Monthly', 'Management', 'Capture KPI trend snapshot', 0, 0, 'kpitrends', 'Open KPI Trends')
    ];
  }
  ...
  function managementPackSummary(ent) {
    ...
    const targetVariance = buildKpiTargetVariance(ent);
    const offTargetControls = targetVariance.filter(row => row.status === 'Off target').length;
    const focusAreas = [];
    ...
    if (offTargetControls) focusAreas.push('KPI target variance');
    return {
      entity: ent,
  ...
  function managementPackExportRows(ent) {
    ...
    return [
      ...
      { Section: 'Executive Summary', Priority: 'Off-target controls', Record: '', Supplier: '', Owner: '', Date: '', Amount: '', Detail: String(summary.offTargetControls) },
      ...buildKpiTargetVariance(ent).map(row => ({
        Section: 'KPI Target Variance',
        Priority: row.status,
        Record: row.metric,
        Supplier: '',
        Owner: row.owner,
        Date: '',
        Amount: row.currentLabel,
        Detail: `Target ${row.targetLabel}; variance ${row.varianceLabel}; action ${row.action}`
      })),
  ...
  function renderManagementPack() {
    ...
    const rows = managementPackRows(ent);
    const summary = managementPackSummary(ent);
    const targetRows = buildKpiTargetVariance(ent);
    const offTargetRows = targetRows.filter(row => row.status === 'Off target');
    viewEl.innerHTML = `${pageHead('Monthly Management Pack', 'One-click control extract for weekly or monthly procurement/logistics review.', `<button class="btn btn-primary" id="pack-export">Export Pack CSV</button>`)}
      <div class="kpi-pill-row">
        ...
        <div class="kpi-pill"><div class="kpi-pill-label">Off target</div><div class="kpi-pill-value">${offTargetRows.length}</div></div>
      </div>
      <div class="mgmt-band">
        ...
        ${miniTable(['Signal','Count'], [
          ...
          `<tr><td>Off-target controls</td><td class="num">${summary.offTargetControls}</td></tr>`
        ], 'No summary signals.')}
      </div>
      <div class="mgmt-band">
        <div class="mgmt-band-head"><div><h3>KPI target variance</h3><span>Draft operating guardrails for the current entity. Tune the targets once business owners approve final SLAs.</span></div><div class="mgmt-actions">${navButton('kpitrends', 'Open KPI Trends')}</div></div>
        ${miniTable(['Status','Area','Metric','Current','Target','Variance','Owner','Action'], targetRows.map(row => `<tr ${row.view ? `onclick="navigate('${row.view}')" style="cursor:pointer"` : ''}><td>${badge(row.status, row.cls)}</td><td>${escapeHtml(row.area)}</td><td>${escapeHtml(row.metric)}</td><td class="num">${escapeHtml(row.currentLabel)}</td><td class="num">${escapeHtml(row.targetLabel)}</td><td class="num">${escapeHtml(row.varianceLabel)}</td><td>${escapeHtml(row.owner)}</td><td>${escapeHtml(row.action)}</td></tr>`), 'No target rows available.')}
      </div>
  ...
  window.PXManagementControls = {
    ...
    buildKpiTargetVariance, buildOperatingChecklist,
